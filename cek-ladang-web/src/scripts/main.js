...
predictForm.elements.plantsFile.addEventListener('change', plantsFileInputHandler);
...
function dropHandler(event) {
  ...
  const plantImage = files[0];  // Updated to 'plantImage'
  predictFormData.set('image', plantImage, plantImage.name);

  previewFile(plantImage);
}

// Handle file by input element
function plantsFileInputHandler(event) {
